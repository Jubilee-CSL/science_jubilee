...
def main(_config, _run):
    ...
    for i, dest_well_obj in enumerate(dest_wells):
        ...
        cam.move_to_get_image(source_well.x, source_well.y - cam.offset[1], cfg["z_imaging"])
        ...
        if duckweed_3d is None:
            logging.warning("No duckweed detected for well %s — skipping.", dest_well_obj.name)
            continue
        ...
        logging.info("Duckweed target: x=%.2f y=%.2f z=%.2f", x_target, y_target, z_target)

        # ── 5. Debug confirmation ────────────────────────────────────────
        if cfg["debug"]:
            ans = input(f"[{dest_well_obj.name}] Confirm target ({x_target:.1f}, {y_target:.1f}, {z_target:.1f})? [y/n/q]: ")
