...
# ======================================================
def get_img_contour(img, min_area_px=10, max_area_px=300, min_circularity=0.5, debug=False):
    b, g, r = cv2.split(img)
...
def main(img, camera, float_radius_mm=25.0):
    ...
    try:
        ...
        cv2.putText(output_img, f"Flotteur ({float_center_3d})", (float_center_2d[0] + 10, float_center_2d[1]), 
    ...
    if duckweed_tracked:
        ...
        cv2.putText(output_img, f"Cible({duckweed_3d})", (duckweed_tracked[0] + 10, duckweed_tracked[1]), 
    ...
    cv2.putText(output_img, text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
