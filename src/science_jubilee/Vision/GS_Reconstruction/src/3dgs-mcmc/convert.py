...
_sparse_dir = args.source_path + "/distorted/sparse"
_subdirs = [
    d for d in os.listdir(_sparse_dir) if os.path.isdir(os.path.join(_sparse_dir, d))
]
# pick the sub-reconstruction with the most registered images (largest images.bin)
_best = max(
    _subdirs,
    key=lambda d: os.path.getsize(os.path.join(_sparse_dir, d, "images.bin")),
    default="0",
)
img_undist_cmd = (
    colmap_command
