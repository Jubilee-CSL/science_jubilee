...
def segment_plant_mask(
    image: np.ndarray, birefnet, transform_image, use_ai: bool = True
) -> np.ndarray:
...
def segment_tray_mask(
    image: np.ndarray, margin_padding_px=20
...
def main(images_path, use_ai=True):
    birefnet = AutoModelForImageSegmentation.from_pretrained(
        "ZhengPeng7/BiRefNet-DIS5K", trust_remote_code=True
    )
    birefnet.to("cuda")
    birefnet.eval()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Segmenter la plante, le cube et les codes ArUco."
    )
    parser.add_argument(
        "--images_path",
        type=str,
    ...
    parser.add_argument(
        "--no_ai",
        action="store_true",
        help="Désactiver rembg (utilise HSV pour la plante)",
    )
