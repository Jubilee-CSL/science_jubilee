...
def process_and_align(input_ply, output_ply, rot_scale=np.array([+3.2, +0.8, 0.0])):
    ...
    pcd_aruco.points = o3d.utility.Vector3dVector(
        np.vstack((x[aruco_indices], y[aruco_indices], z[aruco_indices])).T
    )

    labels_aruco = np.array(
        pcd_aruco.cluster_dbscan(eps=1, min_points=100, print_progress=False)
    )
    unique_labels, counts = np.unique(
        labels_aruco[labels_aruco >= 0], return_counts=True
    )

    if len(unique_labels) < 4:
        raise ValueError(
            f"Seulement {len(unique_labels)} clusters ArUco trouvés (4 attendus)."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rot",
        default=np.array([+3.2, +0.8, 0.0]),
        help="Array contenant les rotations (Pitch, Yaw, Roll) en degrés, séparées par des virgules. Exemple : 3.2,0.8,0.0",
    )
