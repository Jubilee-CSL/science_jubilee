...
def create_mesh_with_alpha_shape(input_ply, output_obj, grid_resolution=0.002, alpha=0.005, decimate_ratio=1.0):
    ...
    if decimate_ratio < 1.0:
        ...
        print(f" Décimation Quadrique... Réduction à {target_faces:,} faces.".replace(',', ' '))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Mesh 2.5D avec Open3D Alpha Shape, Scale et Decimation")

    parser.add_argument("--input", type=str, required=True, help="Nuage .ply d'entrée")
    parser.add_argument("--output", type=str, required=True, help="Maillage .obj de sortie")
    
    # Paramètres de géométrie
    parser.add_argument("--alpha", type=float, default=0.005, help="Valeur Alpha (Distance max entre points). Ex: 0.005 pour 5mm")
    
    # Paramètres de transformation
    parser.add_argument("--decimate", type=float, default=1.0, help="Ratio de triangles à garder (ex: 0.2 = garde 20%)")
