...
CAMERA_MODELS = {
    0: {"name": "SIMPLE_PINHOLE", "num_params": 3, "mapping": lambda p: (p[0], p[0], p[1], p[2])},
    1: {"name": "PINHOLE", "num_params": 4, "mapping": lambda p: (p[0], p[1], p[2], p[3])},
    2: {"name": "SIMPLE_RADIAL", "num_params": 4, "mapping": lambda p: (p[0], p[0], p[1], p[2])},
    3: {"name": "RADIAL", "num_params": 5, "mapping": lambda p: (p[0], p[0], p[1], p[2])},
    4: {"name": "OPENCV", "num_params": 8, "mapping": lambda p: (p[0], p[1], p[2], p[3])},
}
...
def main():
    parser = argparse.ArgumentParser(description="Extracteur de paramètres de caméra depuis les exports COLMAP.")
    parser.add_argument("--path", required=True, help="Chemin vers le fichier cameras.txt/.bin")
    parser.add_argument("--config", default="config.yaml", help="Chemin vers le fichier de configuration de destination")
    ...
    if os.path.isdir(file_path):
        ...
        if os.path.exists(txt_attempt):
            ...
            print("[-] Erreur : Aucun fichier 'cameras.txt' ou 'cameras.bin' trouvé dans ce dossier.")
            return
    ...
    if file_path.endswith(".txt"):
        ...
        print("[-] Erreur : Format de fichier non supporté. Fournissez un fichier .txt ou .bin.")
        return
