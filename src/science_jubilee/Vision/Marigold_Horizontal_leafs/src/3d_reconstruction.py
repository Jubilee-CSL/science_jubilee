...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert Marigold Depthmap into ply point cloud and meshing"
    )
    parser.add_argument("--rgb", type=str, required=True, help="Path to original image")
    parser.add_argument(
        "--depth", type=str, required=True, help="Path to Marigold Depthmap"
    )
    parser.add_argument(
        "--output", type=str, default="output", help="Path to output folder"
    )
    parser.add_argument(
        "--config", default="config.yaml", help="Configuration of the scene"
    )
    parser.add_argument(
        "--create-mesh", type=bool, default=True, help="Choose to create a mesh or not"
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0.0005,
        help="Alpha parameter to the meshing process",
    )
    parser.add_argument(
        "--decimate_ratio",
        type=float,
        default=0.5,
        help="Percentage of original faces to keep after meshing",
    )
