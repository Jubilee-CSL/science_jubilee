...
def main():
    ...
    # ======================================================
    targets = segment_and_target.run_pipeline(
        image_path=images_dir / "latest.jpg",
    ...
    print(
        f"Targets detected: {len(targets)}, veuillez vérifier le fichier output/targets.json pour plus de détails."
    )
    ...
    offset_x = camera.offset[
        0
    ]  # Offset en mm pour ajuster la position finale sur l'axe X
    offset_y = camera.offset[
        1
    ]  # Offset en mm pour ajuster la position finale sur l'axe Y
    offset_z = camera.offset[
        2
    ]  # Offset en mm pour ajuster la position finale sur l'axe Z
    ...
    nav.move_to(
        x=x_depart + target["xyz_mm"][0] + offset_x,
        y=y_depart - target["xyz_mm"][1] + offset_y,
        speed=3000.0,
        wait=True,
    )
    nav.move_to(
        z=z_depart + offset_z - target["xyz_mm"][2] - 5, speed=1000.0, wait=True
    )
