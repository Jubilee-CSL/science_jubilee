...
def depth_to_camera_xyz(depth_map: np.ndarray, u: float, v: float, intrinsics: dict) -> np.ndarray:
...
def segment_plant_mask(image: np.ndarray, use_ai: bool = True) -> np.ndarray:
    if use_ai and remove is not None and new_session is not None:
        try:
            ...
            transparent = remove(pil_image, session=session, alpha_matting=True, alpha_matting_foreground_threshold=240,
...
def segment_tray_mask(image: np.ndarray, margin_padding_px=20) -> np.ndarray:
    ...
    parameters = cv2.aruco.DetectorParameters()   #We dont need for custom parameters (real width, lenght, etc)
...
def estimate_horizontal_targets(image_path: str, depth_path: str, normals_path: str, config: dict, use_ai: bool = True):
    ...
    candidate_mask = cv2.bitwise_and(plant_mask, (normal_z > normal_threshold).astype(np.uint8) * 255)
    candidate_mask = cv2.bitwise_and(candidate_mask, (depth_mm > 0).astype(np.uint8) * 255)
    ...
    depth_consistency_max_std_mm = float(filtering_cfg.get("depth_consistency_max_std_mm", 20.0))
    depth_consistency_radius_px = int(filtering_cfg.get("depth_consistency_radius_px", 3))
    ...
    for label_idx in range(1, num_labels):
        ...
        border_distance = min(left, top, image.shape[1] - (left + width), image.shape[0] - (top + height))
        ...
        local_depths = []
        for y in range(max(0, v - depth_consistency_radius_px), min(h, v + depth_consistency_radius_px + 1)):
            for x in range(max(0, u - depth_consistency_radius_px), min(w, u + depth_consistency_radius_px + 1)):
                if mask[y, x]:
                    local_depths.append(float(np.asarray(depth_mm[y, x]).reshape(-1)[0]))
...
def save_targets(output_dir: str, targets: list, image_path: str, tray_mask: np.ndarray, plant_mask: np.ndarray, depth_mm: np.ndarray):
    ...
    if image is not None:
        ...
        if tray_mask is not None and tray_mask.size > 0:
            ...
            overlay[tray_pixels] = np.clip(overlay[tray_pixels] * 0.7 + tray_color * 0.3, 0, 255).astype(np.uint8)

        if plant_mask is not None and plant_mask.size > 0:
            plant_pixels = plant_mask > 0
            plant_color = np.array([0, 255, 0], dtype=np.uint8)
            overlay[plant_pixels] = np.clip(overlay[plant_pixels] * 0.7 + plant_color * 0.3, 0, 255).astype(np.uint8)

        for target in targets:
            u, v = target["pixel"]
            cv2.circle(overlay, (int(u), int(v)), 8, (0, 0, 255), -1)
            cv2.putText(overlay, str(target["id"]), (int(u) + 8, int(v) - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

            xyz = target["xyz_mm"]
            label = f"x={xyz[0]:.1f} y={xyz[1]:.1f} z={xyz[2]:.1f}"
            cv2.putText(overlay, label, (int(u) + 8, int(v) + 12), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
...
def run_pipeline(image_path: str, output_dir: str, config_path: str = "config.yaml", use_ai: bool = True):
    config = {
        "camera": {},
        "physical": {},
    }
    ...
    targets, tray_mask, plant_mask, depth_mm = estimate_horizontal_targets(image_path, depth_path, normals_path, config, use_ai=use_ai)
    target_path = save_targets(output_dir, targets, image_path, tray_mask, plant_mask, depth_mm)

    print(f"[+] Found {len(targets)} horizontal leaf target(s)")
    for target in targets:
        print(f"    - id={target['id']} xyz={target['xyz_mm']} area_px={target['area_px']}")
    print(f"[+] Target export saved to: {target_path}")
    return targets
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Estimate horizontal leaf surfaces and export camera-relative XYZ coordinates")
    ...
    parser.add_argument("--no-ai", action="store_true", help="Disable rembg-based plant segmentation and use HSV fallback")
    args = parser.parse_args()

    run_pipeline(args.image, args.output, config_path=args.config, use_ai=not args.no_ai)
